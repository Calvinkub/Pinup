...
export default function Map({ landId }: { landId: number }) {
  ...
  // Show polygon UI toolbar when map is ready
  useEffect(() => {
    if (!mapRef.current || !window.longdo || !isMapReady) return;
    
    // Add a small delay to ensure the map UI is fully initialized
    const timer = setTimeout(() => {
      try {
        if (mapRef.current?.Ui?.Toolbar) {
          mapRef.current.Ui.Toolbar.visible(true);
        }
      } catch (error) {
        console.warn('Could not show toolbar:', error);
      }
    }, 100);
  ...
  return (
    <div className="relative">
      <div className="w-full h-[50px] bg-white border-b border-gray-200 px-4 flex items-center justify-end">
        <LayerSelector
